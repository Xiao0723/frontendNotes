...
// document  文档对象
// get 获取 Element  元素  By  根据  Id  标示,如果页面上没有对应的id，此时返回null
...
了解
	...
	// 根据选择器来获取元素，浏览器兼容性问题   IE8以后才支持
	querySelector()     // 只会返回第一个匹配到的元素
	querySelectorAll()	// 返回所有匹配到的元素
```
...
- innerHTML和innerText的区别
  ...
  ```
  功能：获取开始标签和结束标签之间的内容 
     // innerHTML  获取内容的时候，如果内容中有标签，会把标签页获取到，原封不动把内容获取到。
...
// element.onclick  只有事件冒泡
// element.attachEvent 只有两个参数， 只有事件冒泡
```
...
// 获取事件的阶段：1捕获阶段 2目标阶段 3冒泡阶段（了解）e.eventPhase
// 获取真正触发事件的对象  浏览器兼容问题	target = e.target || e.srcElement;
// 获取事件处理函数所属的对象this	e.currentTarget
}
```
...
- event.preventDefault() 取消默认行为    (IE的老版本，非标准方式e.returnValue = false;)
- e.keyCode  键盘码
