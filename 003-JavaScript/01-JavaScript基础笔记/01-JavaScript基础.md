...
JavaScript诞生于1995年，它的出现主要是用于处理网页中的前端验证。
	...
	比如：用户名的长度，密码的长度，邮箱的格式等。（以前网络慢，现在网速快了，所以js不能只停留在做这方面的事情）
...
```js
alert('hello world!')	// 在浏览器窗口弹出

document.write('hello world')	// 在body输出
...
- toString()
  ...
  调用被转换数据类型的toString()方法，该方法不会影响到原变量，它会将转换的结果返回，注意：null与undefined这两个值没有toString()方法

  ```js
  var num = 5;
  console.log(num.toString());	//打印num还是数值5
  ```
...
- parseFloat()
  ...
  > 如果对非String使用parseInt()或者parseFloat()它会先将其转换为String然后在操作

- +，-0等运算

  ```javascript
  var str = '500';
  console.log(+str);		// 取正 变成数值500
  console.log(-str);		// 取负 -500
  console.log(str - 0);  //变成数值500
  var str1 = '500abc';
  console.log(+str1);		// NaN
  console.log(-str1);		// NaN
  console.log(str1 - 0);  //NaN  - 如果一边是数值 一边是字符串， 会先把字符串转换成数值再进行数值的减法运算，如果字符串不能转换成数值类型，输出NaN
  console.log(str1 + 0);  //'500abc0'  + 如果一边是数值 一边是字符串，会先把数值转换字符串 再进行字符串拼接
  var isok = true;
  console.log(+isok);   //1
  ```
...
操作符也叫运算符(operator)，通过运算符可以对一个或多个值进行运算，并获取运算结果，比如：typeof就是运算符，可以获得一个值的类型，它会将该值的类型以字符串的形式返回。
...
```javascript
// for循环的表达式之间用的是;号分隔的，千万不要写成,循环次数确定用for相对简洁，循序次数不确定用while相对简洁
